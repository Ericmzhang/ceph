...
void ECBackend::handle_sub_write_reply(
  ...
  if (ec_write_reply_op.committed) {
    ...
    if (op->cross_zone_write_dispatch_time.contains(from)) {
      ...
      get_parent()->get_logger()->tinc(l_osd_stretch_ec_cross_zone_write_lat, lat);
      get_parent()->get_logger()->hinc(l_osd_stretch_ec_cross_zone_write_lat_hist,
                                       lat.to_nsec(), 0);
    }
  }
...
void ECBackend::handle_sub_read_reply(
  ...
  if (rop.cross_zone_read_dispatch_time.contains(from)) { 
    ...
    if (recv_bytes > 0) {
      ...
      get_parent()->get_logger()->inc(l_osd_stretch_ec_cross_zone_read_recv_bytes,
                                      recv_bytes);
