...
void ECCommon::ReadPipeline::do_read_op(ReadOp &rop) {
  ...
  for (auto &&[pg_shard, read]: messages) {
    ...
    if (ECCommon::is_cross_zone(get_parent(), sinfo, pg_shard.shard)) {
      get_parent()->get_logger()->inc(l_osd_stretch_ec_cross_zone_read_ops);
      get_parent()->get_logger()->inc(l_osd_stretch_ec_cross_zone_read_sent_bytes,
                                      req_bytes);
...
void ECCommon::RecoveryBackend::handle_recovery_push_reply(
  ...
  if (rop.cross_zone_recovery_push_dispatch_time.contains(from)) {
    ...
    get_parent()->get_logger()->tinc(l_osd_stretch_ec_cross_zone_recovery_push_lat,
                                     lat);
    get_parent()->get_logger()->hinc(l_osd_stretch_ec_cross_zone_recovery_push_lat_hist,
                                     lat.to_nsec(), 0);
  }
